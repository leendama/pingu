...
export async function reviewEmailWithModel(reviewer: StructuredReviewer, context: EmailReviewContext, preferences: PreferenceRule[], ownerOperatingBrief?: string, clock?: { now: string; timezone: string }): Promise<EmailReview> {
  ...
  const result = await reviewer.call([
    ...
    "Use ignore for newsletters, promotions, receipts, and mail needing no attention. Use fyi for time-sensitive information that needs attention but no reply. Use decision for a real owner choice without a reply. Use draft only when a reply is appropriate, and provide draft_body only then. Acknowledgements such as thanks, received, or sounds good with no open question or required follow-up are ignore, even when replying to outbound outreach. Check the newest content, not an already answered question in quoted history. Summary: one concise sentence. Rationale: one concise sentence. Draft in the owner's concise natural voice. Never promise facts absent from the email.",
    context.alertMode === "actionable"
      ? "Explicit owner alert policy: alert on the next scan, at any hour, for human replies to the owner's outbound outreach across every project and other personal emails requiring the owner's reply, follow-up, or decision. No urgent deadline or VIP status is needed. These draft/decision outcomes should interrupt even at normal priority. Prefer silence when uncertain: confidence measures whether owner action is truly required; below 0.85 will not interrupt. Exclude acknowledgements, FYIs without owner action, automatic replies, bulk mail and already resolved requests. TLDR summary: name the concrete ask or required follow-up, lowercase prose while preserving names and acronyms. Do not add draft/approval instructions to the alert. This explicit policy takes precedence over inferred preferences."
      : "Mark interrupt only for a same-day deadline, changed meeting, important known sender, or high-confidence time-sensitive decision. Routine items belong in the 9am list.",
