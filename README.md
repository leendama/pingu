...
Your Google token is saved under `PHOTON_DATA_DIR`, or in `data/google-token.json` when no external data directory is set. The assistant can read Calendar, create events, move events, edit events, search Gmail, and create drafts. It sends email after showing the full draft and receiving confirmation in your next message.
