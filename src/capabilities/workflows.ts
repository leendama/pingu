...
import { listWorkflows, saveWorkflow, WORKFLOW_READ_TOOLS } from "../workflows.js";
import { capabilityPlugin, stringArray, stringValue } from "../tools.js";

export function workflowsPlugin(ledger: ProposalLedger) {
  return capabilityPlugin({ id: "workflows", name: "Workflows", description: "Owner-approved reusable preparation workflows.", instructions: ["When the owner asks to make a workflow, define its job, context, output, and only the read tools it needs. Workflows cannot send, draft, edit, delete, or book anything. They always return work for the owner to review."], }, [
    ...
    { schema: { type: "function", name: "list_workflows", description: "List the owner's saved workflows.", strict: true, parameters: { type: "object", additionalProperties: false, properties: {}, required: [] } }, sideEffecting: false, directOnly: true, run: async (_args, context) => ({ output: JSON.stringify({ workflows: listWorkflows(ledger, context.spaceId) }) }) },
    { schema: { type: "function", name: "run_workflow", description: "Run one saved workflow. For the rest of this turn, only its approved read tools are available; return the requested work for owner review.", strict: true, parameters: { type: "object", additionalProperties: false, properties: { name: { type: "string" } }, required: ["name"] } }, sideEffecting: false, directOnly: true, run: async (args, context) => {
      ...
      const workflow = listWorkflows(ledger, context.spaceId).find((item) => item.name.toLowerCase() === name.trim().toLowerCase());
