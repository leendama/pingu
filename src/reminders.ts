...
export async function listReminders(spaceId: string): Promise<Reminder[]> {
  return (await store.read())
    .filter((reminder) => reminder.spaceId === spaceId)
    .sort((left, right) => left.dueAt.localeCompare(right.dueAt));
}

export async function cancelReminder(spaceId: string, reminderId: string): Promise<boolean> {
  return store.update((reminders) => {
    const filtered = reminders.filter(
      (reminder) => reminder.id !== reminderId || reminder.spaceId !== spaceId,
    );
