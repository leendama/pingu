...
const commitmentInput = z.object({ summary: z.string().min(1).max(500), counterparty: z.string().min(1).max(200), owedBy: z.enum(["owner", "other"]), source: z.string().min(1).max(1500) });
const statusInput = z.object({ id: z.string(), status: z.enum(["open", "reply_sent", "completed", "dismissed"]) });
...
export function personalStatePlugin(state: PersonalState) {
  return capabilityPlugin({ id: "personal-state", name: "Tasks and follow-ups", description: "Private durable checkpoints and evidence-backed commitments.", instructions: [
    ...
    { schema: fn("save_commitment", "Record an explicitly agreed obligation and its source. Does not send, schedule, or notify.", commitmentInput), safeAfterUntrusted: true, run: async (a, c) => ({ output: JSON.stringify(await state.saveCommitment(c.spaceId, commitmentInput.parse(a))) }) },
    { schema: fn("update_commitment", "Update a recorded commitment only on owner confirmation or verified source evidence.", statusInput), run: async (a, c) => { const v = statusInput.parse(a); return { output: JSON.stringify(await state.setStatus(c.spaceId, v.id, v.status)) }; } },
  ]);
}
