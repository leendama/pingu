...
export function personalBrainPlugin(root: string) {
  ...
  return capabilityPlugin({ id: "personal-brain", name: "Personal brain", description: "Private source-linked Markdown retrieval and inbox capture.", instructions: [
    "Use search_personal_brain and read_personal_note for the owner's people, prior conversations, principles, lessons, and learning priorities. Use list_personal_note_links to follow existing links to principles and lessons, then read their full evidence. A topical match or existing link is not proof of a causal link or repeated pattern; distinguish direct evidence, recollection, and your suggestion. Cite returned sources. Missing or ambiguous links must not be guessed.",
    ...
    { schema: fn("list_personal_note_links", "Resolve a note's explicit links to other notes, principles and lessons. Reports ambiguous or missing targets rather than guessing. Does not edit the graph.", path), sideEffecting: false, untrustedSource: true, run: async (a) => ({ output: JSON.stringify(await brain.links(path.parse(a).path)) }) },
